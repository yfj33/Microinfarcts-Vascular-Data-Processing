function [RC,HCT] = RBC_HCT_V2(foldername,savefolder,capnum,trialnum)
...
TiffFilename=dir(fullfile(foldername,'*_Ch3_*.ome.tif'));
%tiffFilename={TiffFilename.name};
tiffFilename_full=fullfile(TiffFilename.folder,TiffFilename.name);
% tiffFilename_full=tiffFilename_full{1};
A=imread(tiffFilename_full);
A=uint16(A);
...
HCT(1,1)=RBC_Pixel_Med;%median filter 3*3
HCT(1,2)=RBC_Pixel_Mn;%hct after mean 3*3 filter
HCT(1,3)=RBC_Pixel_Gn;%gaussian filter 1std
HCT(1,4)=RBC_Pixel_Wn;%wiener filter 7
HCT(1,5)=Pixel_Count; %PIxel Count of the image
...
%saveas(fig,fullfile(savefolder,sprintf('Binarized and Filtered Image %d.jpg',gp)));
figname=['capillary',num2str(capnum),' ','trial',num2str(trialnum),' ','HCT Binarization'];
...
RC(1,1)=rbcNum; %RBCnumber by AI
RC(1,2)=rbcFlux;%RBC FLUX by AI
RC(1,3)=M_RBC_Num;%RBCnumber by hand
RC(1,4)=M_RBC_Flux;%RBCflux by hand
RC(1,5)=frameperiod; %FRAMEPeriod for calculate of multiple frams
close all
