function [VA Num K] = LSV(foldername)
% Calculate LS CBFV with input foldername and picture number
% Output mean velocity and angle and plots of each graph
...
VA=zeros(Num,3);%angle and velocity, first number is the number of the pics in the folder
...
%time 
Time=Cali_Factor{k,1};%unit s
P_T= 256; % pixels for time interval
T_Per_Pixel= Time/P_T %unit s/pixel
%distance

D_Per_Pixel=Cali_Factor{k,2}; %unit um/pixel
%calibration value
...
VA(k,1)=Drew_Angle_m;VA(k,2)=Drew_CBFV_m;
