...
%
foldername='C:\Users\Yifu Jin\Desktop\JYF PPT\Project\Stroke\data\5.7 aged\5.7 aged Linescan Info'
...
for a=1:length(week_info)
    ...
    BOX_Flux=boxplot(RBC_FLUX,'position',position,'color',char(color(j)));
    hold on
...
%ax.XAxis.FontSize = 18;
%ax.YAxis.FontSize = 18;
fig3=figure %mean filter HCT 
for m=1:length(week_info)
     for n=1:3
    HCT=hct{m,n,2}
    position=Position(n)+(2*m-1)
    BOX_HCT=boxplot(HCT,'position',position,'color',char(color(j)))
    hold on
    x=ones(size(HCT)).*(position-0.01+(rand(size(HCT))-0.5)/10);
